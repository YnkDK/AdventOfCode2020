...
from dec07 import Regulations


class December07Test(unittest.TestCase):
    _EXAMPLE = """light red bags contain 1 bright white bag, 2 muted yellow bags.
dark orange bags contain 3 bright white bags, 4 muted yellow bags.
bright white bags contain 1 shiny gold bag.
muted yellow bags contain 2 shiny gold bags, 9 faded blue bags.
shiny gold bags contain 1 dark olive bag, 2 vibrant plum bags.
...
dotted black bags contain no other bags."""
    ...
    def test(self):
        regulations = Regulations(December07Test._EXAMPLE)
        bag_colors = regulations.outermost_bag_colors()
        self.assertEqual(4, len(bag_colors))
        self.assertEqual(32, regulations.number_of_bags_to_buy() - 1)
