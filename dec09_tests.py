...
from dec09 import solve, ExchangeMaskingAdditionSystem


class December09Test(unittest.TestCase):
    _EXAMPLE = """35
20
15
25
...
576"""
    ...
    def test_parsing(self):
        system = ExchangeMaskingAdditionSystem(December09Test._EXAMPLE, 5)

        self.assertEqual(35, system.numbers[0])
        self.assertEqual(576, system.numbers[-1])

    def test_find_first_error(self):
        system = ExchangeMaskingAdditionSystem(December09Test._EXAMPLE, 5)

        first_error, error_index = system.find_first_error()

        self.assertEqual(127, first_error)
        self.assertEqual(14, error_index)

    def test_break_encryption(self):
        system = ExchangeMaskingAdditionSystem(December09Test._EXAMPLE, 5)

        encryption_weakness = system.break_encryption(14)

        self.assertEqual(62, encryption_weakness)

    def test_solve(self):
        system = ExchangeMaskingAdditionSystem(December09Test._EXAMPLE, 5)
        gen = solve(system)

        solutions = [solution for solution in gen]

        self.assertEqual(2, len(solutions))
        self.assertEqual(127, solutions[0])
        self.assertEqual(62, solutions[1])
